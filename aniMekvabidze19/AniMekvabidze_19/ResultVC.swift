...
class ResultVC: UIViewController {
    ...
    func setupViews() {
        ...
        btnAgain.topAnchor.constraint(equalTo: btnRestart.bottomAnchor, constant: 40).isActive=true
        btnAgain.centerXAnchor.constraint(equalTo: self.view.centerXAnchor).isActive=true
        btnAgain.widthAnchor.constraint(equalToConstant: 150).isActive=true
        btnAgain.heightAnchor.constraint(equalToConstant: 50).isActive=true
        btnAgain.addTarget(self, action: #selector(btnAgaintAction), for: .touchUpInside)
    }
    ...
    let btnAgain: UIButton = {
        let btn = UIButton()
        btn.setTitle("get higher score", for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        btn.backgroundColor=UIColor.systemIndigo
        btn.layer.cornerRadius=5
        btn.clipsToBounds=true
